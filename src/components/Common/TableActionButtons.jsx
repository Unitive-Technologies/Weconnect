...
const TableActionButtons = ({tableActions}) => {
  ...
  const normalTypeActions = tableActions.filter(action => action.type === "normal")
  const dropdownTypeActions = tableActions.filter(action => action.type === "dropdown")
    ...
    <div>
        ...
        }
      {
        ...
        Object.keys(dropdownTypedActions).map((dropdownName, index) => {
          return (
            <UncontrolledDropdown className="dropdown d-inline-block me-1" key={index}>
              <DropdownToggle
                type="menu"
