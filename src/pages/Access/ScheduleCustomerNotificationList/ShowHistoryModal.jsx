...
const ShowHistoryModal = ({ isOpen, toggleHistoryModal, viewScheduleNoti }) => {
  console.log("Show History Modal in ReasonList" + viewScheduleNoti);
  ...
  const getHistoryDetails = async (e) => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/user/${viewScheduleNoti.id
  ...
  return (
    ...
    >
      <ModalHeader toggle={toggleHistoryModal} tag="h4" position="relative">
        <h4>User History ({viewScheduleNoti.name})</h4>
      </ModalHeader>
      <ModalBody>
        <div
          style={{
