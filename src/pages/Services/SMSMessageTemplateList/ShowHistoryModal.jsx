...
const ShowHistoryModal = ({ isOpen, toggleHistoryModal, SMSMsgTemp }) => {
  console.log("Show History Modal in ReasonList" + SMSMsgTemp)
  ...
  const getHistoryDetails = async (e) => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/sms-message-template/${SMSMsgTemp.id
  ...
  return (
    ...
    >
      <ModalHeader toggle={toggleHistoryModal} tag="h4" position="relative">
        <h4>SMS Message Template History ({SMSMsgTemp.name})</h4>
      </ModalHeader>
      <ModalBody>
        <div
          style={{
