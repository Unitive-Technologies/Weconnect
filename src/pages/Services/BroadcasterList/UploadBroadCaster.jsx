...
import {
    downloadBroadcasterUploadTemplate,
    updateBroadcasterUploadByToken,
    uploadBroadcasterSubmit,
} from "../../../helpers/backend_helper";

const UploadBroadCaster = (props) => {
    const { isOpen, toggleUploadBroadcaster, brodcastStatus, actiontype } = props;
    ...
    function handleAcceptedFiles(files) {
        ...
        updateBroadcasterUploadByToken(uploadTrigger.token, broadcasterSavedTemplatePayload)
            .then((res) => {
                console.log("res in updateBroadcasterUploadByToken:" + JSON.stringify(res));
            })
            .catch((error) => {
                console.log("error in updateBroadcasterUploadByToken:" + error);
            });
    }

    const broadcasterSavedTemplatePayload = {
    ...
    const handleDownloadSampleFile = () => {
        ...
        downloadBroadcasterUploadTemplate(broadcasterDownloadTemplatePayload)
            .then((res) => {
                debugger;
            ...
            .catch((error) => {
                console.log("error in downloadBroadcasterUploadTemplate:" + error);
            });
    };
    ...
    const handleUploadFile = () => {
        ...
        uploadBroadcasterSubmit(uploadTrigger.token, formData)
            .then((res) => {
                // debugger;
                toggleSuccessMsg();
                console.log(
                    "res in uploadBroadcasterFileForInitiatedUserUpload:" + JSON.stringify(res)
                );
                ...
                dispatch(onAddNewBroadCaster(res.data.data));
                toggleUploadBroadcaster();
            })
            .catch((error) => {
        ...
        <>
            ...
            >
                <Toast isOpen={successMsg}>
                    ...
                    <ToastBody>Upload Broadcaster Successfully</ToastBody>
                </Toast>
            </div>
            <Modal
                isOpen={isOpen}
                ...
                tabIndex="-1"
                toggle={toggleUploadBroadcaster}
            >
                {/* <Modal isOpen={modal} toggle={toggle}> */}
                <ModalHeader toggle={toggleUploadBroadcaster} tag="h4">
                    Upload Broadcasters
                </ModalHeader>
                <ModalBody>
                    <Card>
                            ...
                            <div className="text-center mt-4 ">
                                ...
                                >
                                    ...
                                    <button
                                        type="button"
                                        className="btn btn-primary "
                                        onClick={() => toggleUploadBroadcaster()}
                                    >
                                        Cancel
                                    </button>
                                </div>
