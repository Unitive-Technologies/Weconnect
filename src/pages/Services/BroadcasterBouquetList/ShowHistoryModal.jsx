...
const ShowHistoryModal = ({ isOpen, toggleHistoryModal, broadcast }) => {
  console.log("Show History Modal in ReasonList" + broadcast)
  ...
  const getHistoryDetails = async (e) => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/broadcaster-bouque/${broadcast.id
  ...
  return (
    ...
    >
      <ModalHeader toggle={toggleHistoryModal} tag="h4" position="relative">
        <h4>Broadcaster Bouquet History ({broadcast.name})</h4>
      </ModalHeader>
      <ModalBody>
        <div
          style={{
