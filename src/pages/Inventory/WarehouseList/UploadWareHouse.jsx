...
import { Link } from "react-router-dom";
import Dropzone from "react-dropzone";
import { addWareHouseList as onAddWareHoustList } from "/src/store/warehouse/actions";

import { useDispatch } from "react-redux";
import {
  downloadWarehouseUploadTemplate,
  updateWarehouseUploadByToken,
  uploadWarehouseSubmit,
} from "../../../helpers/backend_helper";

const UploadWareHouse = (props) => {
  const { isOpen, toggleUploadWarehouse, warehouseStatus, actiontype } = props;
  ...
  function handleAcceptedFiles(files) {
    ...
    updateWarehouseUploadByToken(uploadTrigger.token, warehouseSavedTemplatePayload)
      .then((res) => {
        console.log("res in updateWarehouseUploadByToken:" + JSON.stringify(res));
      })
      .catch((error) => {
        console.log("error in updateWarehouseUploadByToken:" + error);
      });
  }

  const warehouseSavedTemplatePayload = {
    meta_data: { type: 1, status: parseInt(status) },
    url: "",
  };

  const warehouseDownloadTemplatePayload = {
    meta_data: { type: 1 },
    url: "",
  };
  ...
  const handleDownloadSampleFile = () => {
    ...
    downloadWarehouseUploadTemplate(warehouseDownloadTemplatePayload)
      .then((res) => {
        debugger;
      ...
      .catch((error) => {
        console.log("error in downloadWarehouseUploadTemplate:" + error);
      });
  };
  ...
  const handleUploadFile = () => {
    ...
    uploadWarehouseSubmit(uploadTrigger.token, formData)
      .then((res) => {
        // debugger;
        toggleSuccessMsg();
        console.log(
          "res in uploadWarehouseFileForInitiatedUserUpload:" + JSON.stringify(res)
        );
        ...
        dispatch(onAddWareHoustList(res.data.data));
        toggleUploadWarehouse();
      })
      .catch((error) => {
    ...
    <>
      ...
      >
        <Toast isOpen={successMsg}>
          ...
          <ToastBody>Upload Warehouse Successfully</ToastBody>
        </Toast>
      </div>
      <Modal
        isOpen={isOpen}
      ...
      >
        ...
        <ModalHeader toggle={toggleUploadWarehouse} tag="h4">
          Upload Warehouses
        </ModalHeader>
        <ModalBody>
          <Card>
